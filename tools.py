...
def simulate(agent, envs, steps=0, episodes=0, state=None, label=None):
  ...
  while (steps and step < steps) or (episodes and episode < episodes):
    ...
    if label is not None:
      action, agent_state = agent(obs, done, agent_state, reward, label=label)
    else:
  ...
  # return (step - steps, episode - episodes, done, length, obs, agent_state, reward_all)
...
def sample_episodes_first(episodes, length=None, balance=False, seed=0):
  random = np.random.RandomState(seed)
  while True:
    episode = random.choice(list(episodes.values()))
    if length:
      ...
      if available < 1:
        ...
        continue
      if balance:
        index = min(random.randint(0, 15), available)
      else:
        index = int(random.randint(0, 15))
      episode = {k: v[index: index + length] for k, v in episode.items()}
    yield episode
...
def static_scan_withlabel(fn, inputs, start, label):
  last = start
  indices = range(inputs[0].shape[0])
  flag = True
  for index in indices:
    inp = lambda x: (_input[x] for _input in inputs)
    last = fn(last, *inp(index), label=label)
    if flag:
      if type(last) == type({}):
        outputs = {key: value.clone().unsqueeze(0) for key, value in last.items()}
      else:
