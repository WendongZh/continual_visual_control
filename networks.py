...
class RSSM(nn.Module):
  ...
  def __init__(
      ...
      cell='gru',
      num_actions=None, embed = None, device=None, label_num=4):
    super(RSSM, self).__init__()
    self._stoch = stoch
    self._deter = deter
    self._hidden = hidden
    self._min_std = min_std
    self._layers_input = layers_input # 1
    ...
    inp_layers = []
    if self._discrete:
      inp_dim = self._stoch * self._discrete + num_actions + label_num
    else:
...
class ConvEncoder(nn.Module):
  ...
  def __init__(self, grayscale=False,
               depth=32, act=nn.ReLU, kernels=(4, 4, 4, 4), label_num=4):
    ...
    # shape of label_tmp is 1, 3, 3, 1, 1
    self.label_use = nn.Parameter(data=label_tmp, requires_grad=False)

  def __call__(self, obs, label):
...
class ConvDecoder(nn.Module):
  ...
  def __init__(
    ...
    # shape of label_tmp is 1, 1, 3, 3
    self.label_use = nn.Parameter(data=label_tmp, requires_grad=False)

  def __call__(self, features, label, dtype=None):
...
class DenseHead(nn.Module):
  ...
  def __init__(
      self, inp_dim,
      shape, layers, units, act=nn.ELU, dist='normal', std=1.0, label_num=4):
    ...
    for index in range(self._layers):
      if index == 0:
        mean_layers.append(nn.Linear(inp_dim + label_num, self._units))
...
class ActionHead(nn.Module):
  ...
  def __init__(
      ...
      init_std=0.0, min_std=0.1, action_disc=5, temp=0.1, outscale=0, label_num=4):
    ...
    for index in range(self._layers):
      if index == 0:
        pre_layers.append(nn.Linear(inp_dim + label_num, self._units))
  ...
  def __call__(self, features, label, dtype=None):
    ...
    # print(features.size())
    # print(label_use.size())

    shape_tmp = features.size()
    label_use = self.label_use[:, label]

    if len(shape_tmp) == 2:
      label_use = label_use.repeat(shape_tmp[0], 1)
    elif len(shape_tmp) == 3:
      label_use = label_use.unsqueeze(0).repeat(shape_tmp[0], shape_tmp[1], 1)
    else:
      print(shape_tmp)
    features = torch.cat((label_use.detach(), features), -1)
